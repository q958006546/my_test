...
public class ElasticsearchUtil {
    ...
    public XContentBuilder generateBuilder(Class clazz) throws IOException {
        ...
        for (Field f : declaredFields) {
            if (f.isAnnotationPresent(com.lzyd.es.utils.Field.class)) {
                // 获取注解
                com.lzyd.es.utils.Field  declaredAnnotation = f.getDeclaredAnnotation(com.lzyd.es.utils.Field.class);
                ...
                if (declaredAnnotation.type() == FieldType.OBJECT) {
                    ...
                    // 遍历该对象中的所有属性
                    for (Field f2 : df2) {
                        if (f2.isAnnotationPresent(com.lzyd.es.utils.Field.class)) {
                            // 获取注解
                            com.lzyd.es.utils.Field declaredAnnotation2 = f2.getDeclaredAnnotation(com.lzyd.es.utils.Field.class);
